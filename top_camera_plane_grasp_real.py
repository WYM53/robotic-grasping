...
class PlaneGraspClass:
    def __init__(self, saved_model_path=None, use_cuda=True, visualize=False, include_rgb=True, include_depth=True, output_size=300):
        ...
        self.ur_robot = Ur5e_Robot(tcp_host_ip="192.168.1.12", tcp_port=50002, is_use_robotiq85=True, is_use_camera=True)
    ...
    def generate(self):
        ...
        # depth[depth >1.2]=0 # distance > 1.2m ,remove it  处理深度噪声，将 >1.2m 的信息均设置为0

        depth= np.expand_dims(depth, axis=2) # 深度图维度扩展
        x, depth_img, rgb_img = self.cam_data.get_data(rgb=rgb, depth=depth)
        rgb = cv2.cvtColor(rgb,cv2.COLOR_BGR2RGB)

        with torch.no_grad():
            xc = x.to(self.device) # 放入GPU
            pred = self.model.predict(xc) # 得到预测结果
        q_img, ang_img, width_img = post_process_output(pred['pos'], pred['cos'], pred['sin'], pred['width'])

        grasps = detect_grasps(q_img, ang_img, width_img) # 得到抓取姿态
        if len(grasps) ==0:
        ...
        angle = np.asarray([0, 0, grasps[0].angle]) # 把抓取角度转换为rpy
        angle.shape = (3, 1)
